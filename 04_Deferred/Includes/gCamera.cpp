...
void gCamera::Resize(int _w, int _h)
{
	...
	SetProj(glm::radians(60.0f), _w / (float)_h, this->getNearPlane(), this->getFarPlane());
}
...
glm::mat4 gCamera::GetFrustumRange(float _zn, float _zf) const
{
	return glm::perspective(angle, aspect, _zn, _zf) * m_viewMatrix;
}
