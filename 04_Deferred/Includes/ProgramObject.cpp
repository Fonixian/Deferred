...
void ProgramObject::setTextureArray(const char* _uniform, int _sampler, GLuint _textureID) {
	glActiveTexture(GL_TEXTURE0 + _sampler);
	glBindTexture(GL_TEXTURE_2D_ARRAY, _textureID);
	glUniform1i(GetLocation(_uniform), 3);
}
void ProgramObject::SetUBO(const char* _uniform, int _sampler, GLuint _buffer) {
	GLuint block_index = glGetUniformBlockIndex(m_id, _uniform);
	glBindBufferBase(GL_UNIFORM_BUFFER, block_index, _buffer);
	glUniformBlockBinding(m_id, block_index, _sampler);
}
